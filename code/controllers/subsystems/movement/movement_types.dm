...
/**
 * Used for following jps/pathfinding defined paths.
 *
 * Returns TRUE if the loop sucessfully started, or FALSE if it failed
 *
 * Arguments:
...
/datum/move_loop/has_target/jps
	...
	var/max_path_length = 30
	///Minimum distance to the target before path returns
	var/minimum_distance = 0
	///An ID card representing what access we have and what doors we can open
	...
	var/repath_cooldown = 0

/datum/move_loop/has_target/jps/setup(delay, timeout, atom/chasing, repath_delay, max_path_length, minimum_distance, obj/item/weapon/card/id/id, simulated_only, turf/avoid, skip_first)
	. = ..()
	if(!.)
		return
	if(repath_delay)
	...
	if(istype(id, /obj/item/weapon/card/id))
		RegisterSignal(id, COMSIG_PARENT_QDELETING, .proc/handle_no_id)
...
/datum/move_loop/has_target/jps/proc/recalculate_path()
	if(world.time < repath_cooldown)
		return
	repath_cooldown = world.time + repath_delay
...
/proc/get_path_to(atom/movable/caller, atom/target, max_distance = 30, mintargetdist = 0, id = null, simulated_only = FALSE, turf/avoid = null, skip_first = FALSE)
	var/turf/start = get_turf(caller)
	var/turf/end = get_turf(target)
	if(!start || !end)
		return list()
	var/list/path = AStar(start, end, /turf/proc/AdjacentTurfs, /turf/proc/Distance, max_nodes = max_distance * 2, max_node_depth = max_distance, min_target_dist = mintargetdist, id = id, exclude = avoid)
	if(!path)
		return list()
	if(skip_first && path.len)
		path.Cut(1, 2)
	return path
 ...
 * moving - The atom we want to move
 * chasing - The atom we want to move towards
 * min_dist - the closest we're allowed to get to the target
 * delay - How many deci-seconds to wait between fires. Defaults to the lowest value, 0.1
 * timeout - Time in deci-seconds until the moveloop self expires. Defaults to infinity
 * subsystem - The movement subsystem to use. Defaults to SSmovement. Only one loop can exist for any one subsystem
 * priority - Defines how different move loops override each other. Lower numbers beat higher numbers, equal defaults to what currently exists. Defaults to MOVEMENT_DEFAULT_PRIORITY
 ...
 *
**/
/datum/controller/subsystem/move_manager/proc/move_to(moving, chasing, min_dist = 0, delay, timeout, subsystem, priority, flags, datum/extra_info)
 ...
 *
**/
/datum/controller/subsystem/move_manager/proc/move_away(moving, chasing, max_dist = 0, delay, timeout, subsystem, priority, flags, datum/extra_info)
...
/datum/move_loop/disposal_holder/move()
	...
	var/atom/old_loc = moving.loc
	var/obj/structure/disposalpipe/curr = holder.loc
	if(istype(curr))
		var/obj/structure/disposalpipe/last = curr
		curr = curr.transfer(holder)
		if(QDELETED(holder))
			return FALSE
		if(!curr)
			last.expel(holder, holder.loc, holder.dir)
			return FALSE
	return old_loc != moving.loc
