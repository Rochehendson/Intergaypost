...
/datum/extension/labels/proc/AddLabel(label, mob/user)
	...
	if (user)
		user.visible_message(
			"\<span class='italic'>The [user] attaches a label to \a [atom_holder].</span>",
			"<span class='italic'>You label \the [atom_holder] with \"[label]\".</span>"
		)
...
/datum/extension/labels/proc/RemoveLabel(label, mob/user)
	...
	if (user)
		user.visible_message(
			"\<span class='italic'>The [user] removes a label from \a [atom_holder].</span>",
			"<span class='italic'>You remove the \"[label]\" label from \the [atom_holder].</span>"
		)
	var/old_name = atom_holder.name
