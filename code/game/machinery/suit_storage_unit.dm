...
/obj/machinery/suit_storage_unit/attack_hand(mob/user as mob)
	...
	if(panelopen) //The maintenance panel is open. Time for some shady stuff
		...
		dat+= text("<font color ='black'>A small dial with a small lambda symbol on it. It's pointing towards a gauge that reads []</font>.<BR> <font color='blue'><a href='byond://?src=\ref[];toggleUV=1'> Turn towards []</A></font><BR>",(issuperUV ? "15nm" : "185nm"),src,(issuperUV ? "185nm" : "15nm") )
		dat+= text("<font color ='black'>A thick old-style button, with 2 grimy LED lights next to it. The [] LED is on.</font><BR><font color ='blue'><a href='byond://?src=\ref[];togglesafeties=1'>Press button</a></font>",(safetieson? "<font color='green'><B>GREEN</B></font>" : "<font color='red'><B>RED</B></font>"),src)
		dat+= text("<HR><BR><a href='byond://?src=\ref[];mach_close=suit_storage_unit'>Close panel</A>", user)
		...
		if(!isbroken)
			...
			if(boots && isopen)
				dat+=text("<a href='byond://?src=\ref[];dispense_boots=1'>Dispense footwear</A><BR>",src)
			dat+= text("<font color='black'>Tank storage compartment: <B>[]</B></font><BR>",(tank ? tank.name : "</font><font color ='grey'>No air tank detected.") )
			if(tank && isopen)
				dat+=text("<a href='byond://?src=\ref[];dispense_tank=1'>Dispense air tank</A><BR>",src)
			...
			dat+= text("<HR><font color='black'>Unit is: [] - <a href='byond://?src=\ref[];toggle_open=1'>[] Unit</A></font> ",(isopen ? "Open" : "Closed"),src,(isopen ? "Close" : "Open"))
			if(isopen)
				dat+="<HR>"
			else
				dat+= text(" - <a href='byond://?src=\ref[];toggle_lock=1'><font color ='orange'>[] Unit</A></font><HR>",src,(islocked ? "Unlock" : "Lock") )
			...
			dat += text("<BR><BR><a href='byond://?src=\ref[];mach_close=suit_storage_unit'>Close control panel</A>", user)
			...
			dat+= text("<HR><a href='byond://?src=\ref[];mach_close=suit_storage_unit'>Close control panel</A>", user)
