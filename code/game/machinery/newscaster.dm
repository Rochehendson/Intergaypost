...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if(href_list["set_channel_name"])
			...
			src.msg = sanitize(input(usr, "Write your Feed story", "Network Channel Handler", "") as message|null)
			src.updateUsrDialog()
			...
			src.msg = sanitize(input(usr, "Provide the a description of the Wanted person and any other details you deem important", "Network Security Handler", "") as message|null)
			src.updateUsrDialog()
...
obj/item/weapon/newspaper/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/pen))
		if(src.scribble_page == src.curr_page)
			...
			var/s = sanitize(input(user, "Write something", "Newspaper", "") as text|null)
			if (!s)
				return
