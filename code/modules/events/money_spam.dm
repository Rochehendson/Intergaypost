	...
	if(useMS)
		if(prob(5))
			...
			switch(pick(1,2,3,4,5,6,7))
				...
				if(2)
					sender = pick(300;"QuickDatingSystem",200;"Find your russian bride",50;"Serbian beauties are waiting",50;"Find your secret corporate crush",50;"Beautiful scientist brides")
				...
				if(6)
					sender = pick("[company_name] Morale Divison","Feeling Lonely?","Bored?","www.SmokingBombs.tc")
					message = pick("The [company_name] Morale Division wishes to provide you with quality entertainment sites.",\
					"SmokingBombs.tc is a xenophillic website endorsed by TTC for the use of male crewmembers among it's many stations and outposts.",\
					"SmokingBombs.tc only provides the higest quality of male entertaiment to [company_name] Employees.",\
